...
const BASE_URL =
  (process.env.PLANS_BACKEND_BASE_URL || "").trim().replace(/\/+$/, "") ||
  "http://localhost:8000";
...
async function backendFetch(
  ...
  if (!response.ok) {
    ...
    const detail =
      (bodyText && safeParseJson(bodyText)?.detail) ||
      bodyText ||
      `HTTP ${response.status}`;
    throw new Error(
      `Backend error for ${init.method || "GET"} ${url}: ${detail}`
    );
  }
...
server.tool(
  "create_plan",
  {
    ...
    deadline: z
      ...
      .optional()
      .describe(
        "Optional deadline (ISO 8601 date or datetime). Null/omit if not set."
      ),
    schedule_slot: z
      .string()
      .nullable()
      .optional()
      .describe(
        'Optional schedule window as "HH:MM-HH:MM" or similar. Null/omit if not set.'
      ),
    description: z
      .string()
      .nullable()
...
server.tool(
  "update_plan",
  {
    ...
    deadline: z
      ...
      .optional()
      .describe(
        "New deadline (ISO 8601) or null to clear. Omit field to leave unchanged."
      ),
    schedule_slot: z
      .string()
      .nullable()
      .optional()
      .describe(
        'New schedule window string or null to clear. Omit to leave unchanged.'
      ),
    description: z
      .string()
      .nullable()
      .optional()
      .describe(
        "New description string or null to clear. Omit to leave unchanged."
      ),
  },
  async (args) => {
    const {
...
server.tool(
  "create_habit",
  {
    ...
    previous_label: z
      ...
      .optional()
      .describe(
        'Optional previous label, e.g. "No history yet". Defaults based on backend.'
      ),
    description: z
      .string()
      .nullable()
...
server.tool(
  "update_habit",
  {
    ...
    previous_label: z
      ...
      .optional()
      .describe(
        'New previous label or null to clear (omit to keep existing).'
      ),
    description: z
      .string()
      .nullable()
      .optional()
      .describe(
        "New description or null to clear (omit to keep existing)."
      ),
  },
  async (args) => {
    const {
