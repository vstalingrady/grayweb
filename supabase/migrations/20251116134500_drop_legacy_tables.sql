...
--
-- Safety: destructive cleanup only runs when explicitly confirmed.
do $$
begin
    if current_setting('app.legacy_cleanup_confirmed', true) = 'true' then
        ...
        raise notice 'Skipping legacy table cleanup. Set app.legacy_cleanup_confirmed=true to execute drops.';
    end if;
end $$;
